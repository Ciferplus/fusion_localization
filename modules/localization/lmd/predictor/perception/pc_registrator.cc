...
double PCRegistrator::ComputeError2(
    const std::vector<PCSourcePoint>& source_points,
    const apollo::common::PointENU& position, double heading) const {
  double error = 0.0;
  std::size_t not_found = 0;
  ...
  PCMapIndex near_ni = 0;
  bool find_nearest_flag = false;
  auto p_last = source_points[0];
  for (const auto& p : source_points) {
    // FLU to ENU
    double enu_x, enu_y;
    RotateAxis(-heading, p.position.x(), p.position.y(), &enu_x, &enu_y);
    ...
    if (nearest_p.next != (PCMapIndex)-1) {
      const auto& next_p = map_->Point(nearest_p.next);
      auto d_map_point = std::sqrt(
          std::pow((next_p.position.x() - nearest_p.position.x()), 2.0) +
          std::pow((next_p.position.y() - nearest_p.position.y()), 2.0));

      auto d_source_point =
          std::sqrt(std::pow((enu_position.x() - p_last.position.x()), 2.0) +
                    std::pow((enu_position.y() - p_last.position.y()), 2.0));

      d2 = std::sqrt(std::pow(d_map_point - d_source_point, 2.0));
    }
