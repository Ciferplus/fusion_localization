...
bool PredictorOutput::EstimatedPosesFilter() {
  auto latest_it = predicted_.Latest();

  double timestamp_sec_0, timestamp_sec_1;
  if (latest_it != predicted_.end()) {
    Pose pose_0, pose_1, pose_2;
    timestamp_sec_0 = latest_it->first;
    pose_0.CopyFrom(latest_it->second);

    predicted_.Pop();
    auto latest_it_1 = predicted_.Latest();
    if (latest_it_1 == predicted_.end()) {
      predicted_.Push(timestamp_sec_0, pose_0);
      return true;
    }
    timestamp_sec_1 = latest_it_1->first;
    pose_1.CopyFrom(latest_it_1->second);

    predicted_.Pop();
    auto latest_it_2 = predicted_.Latest();
    if (latest_it_2 == predicted_.end()) {
